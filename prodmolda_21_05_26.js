...
// 
            // Coloana D / Simbol U.M. pentru randurile dinamice se completeaza automat
            // dupa codul PRODMOLD si trebuie sa ramana doar pentru citire.
            var unitMeasureSelector = 'input[field="dec_dinamicTable_row_cd"], input[name^="dec_dinamicTable_row_cd"]';

            function setUnitMeasureReadonly(scope) {
                jQuery(unitMeasureSelector, scope || document)
                    .prop('readonly', true)
                    .attr('readonly', 'readonly')
                    .addClass('readonly');
            }

            setUnitMeasureReadonly(context);

            jQuery('#mywebform-content')
                .off('paste.prodmoldaReadonlyUnit drop.prodmoldaReadonlyUnit beforeinput.prodmoldaReadonlyUnit', unitMeasureSelector)
                .on('paste.prodmoldaReadonlyUnit drop.prodmoldaReadonlyUnit beforeinput.prodmoldaReadonlyUnit', unitMeasureSelector, function (e) {
                    jQuery(this).prop('readonly', true).attr('readonly', 'readonly');
                    e.preventDefault();
                })
                .off('focus.prodmoldaReadonlyUnit mousedown.prodmoldaReadonlyUnit', unitMeasureSelector)
                .on('focus.prodmoldaReadonlyUnit mousedown.prodmoldaReadonlyUnit', unitMeasureSelector, function () {
                    jQuery(this).prop('readonly', true).attr('readonly', 'readonly');
                });
            ...
            if (window.MutationObserver) {
                ...
                if (dinamicTable && !dinamicTable.prodmoldaReadonlyUnitObserver) {
                    dinamicTable.prodmoldaReadonlyUnitObserver = new MutationObserver(function () {
                        setUnitMeasureReadonly(document);
                    });

                    dinamicTable.prodmoldaReadonlyUnitObserver.observe(dinamicTable, {
                        childList: true,
                        subtree: true
                    });
                }
